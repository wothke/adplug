...
char CmusPlayer::load(const std::string &filename, const CFileProvider &fp)
{
	...
	if (!insts)
	{
		...
		if (bankload == loadPending) { return loadPending; }
...
char CmusPlayer::LoadTimbreBank(const std::string fname, const CFileProvider &fp)
{
...
#ifdef EMSCRIPTEN
		int r= adlib_request_file(fname.c_str());	// check reason
		if (r <0) {
			return loadPending; // file not ready
		}
#endif			
		return false;
	}
...
char CmusPlayer::FetchTimbreData(const std::string fname, const CFileProvider &fp)
{
	binistream *f = fp.open(fname);
	if (!f) {
		#ifdef DEBUG
